...
func delete(client client.Client, args []string) (string, error) {
	...
	fmt.Println(cr)
	if cr.Code != 0 {
		return "", fmt.Errorf("failed to delete instance, error code: %d, error message: %s", cr.Code, cr.Desc)
	}

	return cases.Title(language.Und).String(args[0]) + " instance deleted successfully ", nil
}
