...
func _start(rid uint32) int32 {
	...
	easyjson.Unmarshal([]byte(message), &student)
	log("wasm get struct.name from json >> " + student.Name)
	log("wasm change student name to Jane ")
	student.Name = "Jane"
	log("wasm get new name from the struct >> " + student.Name)
	msg, err := easyjson.Marshal(student)
	log("wasm get json from struct >> " + string(msg))
	return 0
}
